...
<div class="container my-5 py-3 shadow-lg bg-dark">
    <div class="row mb-3">
        <h2 class="text-center pagesTitles text-white ">Ajouter&nbsp;une&nbsp;pop</h2>
    </div>
    <div class="row">
        <?php require_once '../include/adminSidebar.php'; ?>
    </div>
    <form class="row my-3 gap-3" method="POST" action="admin_ajout_pops" enctype="multipart/form-data">
        <div class="d-md-flex gap-3 justify-content-around">
            ...
            <div class="col-md-5 mt-md-0 mt-3 form-floating">
                <input class="form-control <?= isset($formErrors['tags']) ? 'is-invalid' : '' ?>" value="<?= !isset($formErrors['tags']) && !empty($_POST['tags']) ? $_POST['tags'] : '' ?>" type="text" name="tags" id="tags" placeholder=" ">
        ...
        <div class="d-md-flex gap-3 justify-content-around">
            ...
            <div class="col-md-11 form-floating">
                <input class="form-control <?= isset($formErrors['brandInput']) ? 'is-invalid' : '' ?>" value="<?= !isset($formErrors['brandInput']) && !empty($_POST['brandInput']) ? $_POST['brandInput'] : '' ?>" type="text" list="brandsList" name="brandInput" id="brandInput" placeholder=" ">
        ...
        <div class="d-md-flex">
            <div class="col-md-11  mx-auto form-floating">
                <input class="form-control <?= isset($formErrors['reference']) ? 'is-invalid' : '' ?>"value="<?= !isset($formErrors['reference']) && !empty($_POST['reference']) ? $_POST['reference'] : '' ?>" type="text" name="reference" id="reference" placeholder=" ">
