...
class pops extends database
{
    ...
    {
        $query = 'SELECT p.name AS name,`tags`,`reference`,`officialPopImageInTheBox`,`officialPopImageOutBox`,b.name As bName
        FROM `s4u3u_pops` AS p
        INNER JOIN `s4u3u_brands` AS b
        ON p.id_brands=b.id
        where p.id=:id;';
    ...
    public function deletePopsById_brands()
    {
        $query = 'DELETE FROM `s4u3u_pops` WHERE id_brands=:id_brands;';
        $queryPrepare = $this->db->prepare($query);
        $queryPrepare->bindValue(':id_brands', $this->id_brands, PDO::PARAM_INT);
        return $queryPrepare->execute();
    }
